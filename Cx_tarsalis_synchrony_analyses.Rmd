...
Surveillance records were binned into continuous monthly bins (`tBin`), and mean number of mosquitoes per trap hour per month (mmpth) was calculated for each of the included NEON surveillance sites (`data/cx_tar_16_19_including_NAs.csv`). Then, we replaced `NA` values with `0` in preparation of the analyses (`data/input/cx_tar_16_19_13jan24.csv`).
...
```{r NEON temporal coverage plot, message = F, warning = F, echo = F, eval = T, include = T}
...
h2b <- ggplot(sur_slope, aes(slope_surrog2))+
  ...
  labs(subtitle="Approx. annual (10 to 14 month avg.) timescale") +
  theme_bw() +
